...
void Engine::spawActorPreset(const int t_x, const int t_y, const ActorPreset t_actor_preset)
{
	s_engine->m_actors.emplace_back(Actor::s_builder.createActor(t_x, t_y, t_actor_preset).construct());
}
...
void Engine::init()
{
	...
	// Make the player
	Engine::spawActorPreset(0, 0, ActorPreset::PLAYER);
	m_player = m_actors.back().get();

	// Add stairs
	m_actors.emplace_back(Actor::s_builder.createActor(0, 0, '>', "stairs", TCODColor::white, true, false).construct());
