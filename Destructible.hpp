...
public:
    ...
    float takeDamage(Actor *t_owner, const DamageType t_dmg_type, float t_damage);
...
{
public:
    MonsterDestructible(const float t_maxHp, const float t_defense, const char *t_corpse_name, const int t_xp);
...
{
public:
    PlayerDestructible(const float t_maxHp, const float t_defense, const char *t_corpse_name, const int t_xp = 0);
    void die(Actor *t_owner);
    void save(TCODZip &t_zip);
};
